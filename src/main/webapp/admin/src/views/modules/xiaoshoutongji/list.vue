<template>
  <div class="main-content">
                ...
                width="50">
                    ...
                    <template slot-scope="scope">
                      {{scope.row.shangpinmingcheng}}
                                                            ...
                                                            <el-table-column
                    prop="xiaoshouzongshu"
                    header-align="center"
                    align="center"
                    sortable
                    label="销售总数">
                    <template slot-scope="scope">
                      {{scope.row.xiaoshouzongshu}}
                    </template>
                </el-table-column>
                                                            <el-table-column
                    prop="riqi"
    ...
    <add-or-update v-if="addOrUpdateFlag" :parent="this" ref="addOrUpdate"></add-or-update>
      ...
      width="800">  
                                                  <div id="xiaoshouzongshuChart" style="width:100%;height:600px;"></div>
...
export default {
  ...
  methods: {
      ...
      this.$nextTick(()=>{
                                         ...
                                         //  xiaoshouzongshu
                                                                  // xcolumn ycolumn shangpinmingcheng xiaoshouzongshu
                var xiaoshouzongshuChart = this.$echarts.init(document.getElementById("xiaoshouzongshuChart"),'macarons');
        this.$http({
            url: `value/xiaoshoutongji/shangpinmingcheng/xiaoshouzongshu`,
            method: "get"
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                    ...
                    var option = {};
                        ...
                        series: [{
                            ...
                            type: 'bar'
                                        ...
                                        // 使用刚指定的配置项和数据显示图表。
                    xiaoshouzongshuChart.setOption(option);
                    //根据窗口的大小变动图表
                    window.onresize = function() {
                        xiaoshouzongshuChart.resize();
                    };
                }
            }
        });
